...
public class TrainTest {
    ...
    public static void checkRepresentationInvariant(Train train) {
        ...
        assertThat("Number of wagons in the train should not exceed engine capacity",
                train.getNumberOfWagons(), lessThanOrEqualTo(train.getENGINE().getMAX_WAGONS()));
    }
    ...
    @Test
    public void T11E_FirstWagonIsNotChangedWhenWagonShouldNotAttach() {
        final Locomotive LIMITED_CAPACITY_LOCOMOTIVE = new Locomotive(1234, 1);
        final Train LIMITED_TRAIN = new Train(LIMITED_CAPACITY_LOCOMOTIVE, "test", "test2");
        final PassengerWagon passengerWagon = new PassengerWagon(12, 45);

        LIMITED_TRAIN.setFirstWagon(passengerWagon);

        LIMITED_TRAIN.setFirstWagon(passengerWagon1);

        assertEquals(LIMITED_TRAIN.getFirstWagon(), passengerWagon);
    }
    ...
    public void T13_checkCumulativeWagonPropertiesOnTrain() {
        ...
        // check final wagon
        assertEquals( 40, ((PassengerWagon) passengerTrain.getLastWagonAttached()).getNUMBER_OF_SEATS());
    ...
    @Test
    public void T16E_IsOnCorrectPositionAfterInsertOnPosition() {
        assertTrue(trainWithoutWagons.insertAtFront(passengerWagon1));

        final Wagon PASSENGER_WAGON_4 = new PassengerWagon(9000, 50);
        final Wagon PASSENGER_WAGON_5 = new PassengerWagon(9001, 1);

        PASSENGER_WAGON_4.attachTail(PASSENGER_WAGON_5);

        assertTrue(trainWithoutWagons.insertAtPosition(3, PASSENGER_WAGON_4));

        assertEquals(trainWithoutWagons.findWagonAtPosition(3), PASSENGER_WAGON_4);
    }
    ...
    @Test
    public void T17E_cantMoveOneWagonToFullTrain() {
        final Locomotive limitedCapacityLocomotive = new Locomotive(132, 1);
        final Train limitedCapacityTrain = new Train(limitedCapacityLocomotive, "amsterdam", "london");
        limitedCapacityTrain.setFirstWagon(new PassengerWagon(12, 251));

        assertFalse(passengerTrain.moveOneWagon(passengerTrain.getLastWagonAttached().getID(), limitedCapacityTrain));
    }
